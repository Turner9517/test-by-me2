...
const clearInvoices = async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1 && mongoose.connection.db) {
      ...
      const invoices = jsonDb.getLocalInvoices();
      const toDelete = invoices.filter(inv => inv.user === req.user._id);
      for (const invoice of toDelete) {
        if (invoice.fileUrl && fs.existsSync(invoice.fileUrl)) {
          fs.unlinkSync(invoice.fileUrl);
        }
      }
      const remaining = invoices.filter(inv => inv.user !== req.user._id);
      jsonDb.saveLocalInvoices(remaining);
    }
    res.json({ success: true, message: "All invoice history cleared" });
  } catch (error) {
    res.status(500).json({ message: "Unable to clear history", error: error.message });
  }
};
